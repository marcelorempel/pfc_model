...
import xarray as xr
from .basics_setup import membranetuple
...
def set_membr_params(membr_params, set_new, set_current, multi_param_transf, 
    ...
    for param in basics.membr.names:
        param_transf = multi_param_transf.loc[dict(param=param)]
        lambda_ = multi_lambda_.loc[dict(param=param)]
        minparam_inv = multi_minparam_inv.loc[dict(param=param)]
        
        param_inv = inv_transform(param_transf, lambda_, minparam_inv)                  
        membr_params.loc[dict(param=param, cell_index=set_new)] = param_inv
    ...
    #AQUI_2
    for param in basics.membr.names:
        out_min = (membr_params.loc[dict(param=param, cell_index=set_current)]
                    < basics.membr.min.loc[dict(param=param, group=group)])
        out_max = (membr_params.loc[dict(param=param,cell_index=set_current)] 
                    > basics.membr.max.loc[dict(param=param, group=group)])
...
def _set_syn_stsp(new_idc, stsp_kinds, stsp_values, stsp_params):
    ...
    for name_idc in range(len(stsp_kinds)):
        stsp_par = stsp_params.loc[dict(kind=stsp_kinds[name_idc])]
        idc_curr = np.arange(stsp_idc_limits[name_idc], 
                             stsp_idc_limits[name_idc+1])

        syn_params.loc['U', idc_curr] = _random_param(
            len(idc_curr), float(stsp_par.loc['U_mean'].values), 
            float(stsp_par.loc['U_std'].values),    0,    1, 'normal')   
        
        syn_params.loc['tau_rec', idc_curr] = _random_param(
            len(idc_curr), float(stsp_par.loc['tau_rec_mean'].values),
            float(stsp_par.loc['tau_rec_std'].values), 0, 1500, 'normal')
        
        syn_params.loc['tau_fac', idc_curr] = _random_param(
            len(idc_curr), float(stsp_par.loc['tau_fac_mean'].values), 
            float(stsp_par.loc['tau_fac_std'].values), 0, 1500, 'normal')
