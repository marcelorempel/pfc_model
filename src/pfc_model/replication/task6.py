...
@time_report()
def task6(simulation_dir, Iexc_arr, Iinh_arr, seed=None, transient=1000, 
          duration=11000):
    ...
    for trial in range(len(Iexc_arr)*len(Iinh_arr)):
        ...
        PC_len = len(cortex.neuron_idcs(('IN', 0)))
        ...
        IN_result.append(len(spikingIN)*100/PC_len)
        ...
        with open(os.path.join(simulation_dir, 'Iexc_applied.txt'), 'a') as f:
            print(Iexc_applied[-1], end=', ', file=f)
        with open(os.path.join(simulation_dir, 'Iinh_applied.txt'), 'a') as f:
            print(Iinh_applied[-1], end=', ', file=f)
        for i in range(len(results_list)):
            result = results_list[i]
            name = name_list[i]
            with open(os.path.join(simulation_dir, name+'.txt'), 'a') as f:
                print(result[-1], end=', ', file=f)
        
    with open(os.path.join(simulation_dir, 'varying_I.txt'), 'w') as f:
        print('Iexc_applied\n', file=f)
        print(Iexc_applied, file=f)
        print('-'*40, file=f)
        print('Iinh_applied\n', file=f)
        print(Iinh_applied, file=f)
        print('-' *40, file=f)
        for i in range(len(results_list)):
            result = results_list[i]
            name = name_list[i]
    ...
    _fig11(Iexc_arr, Iinh_arr, results_list2, name_list2,
             label_list2, simulation_dir)

    
              
def _fig11(Iexc_arr, Iinh_arr, results_list, name_list,
             label_list, save_dir):
    
    if not os.path.isdir(os.join(save_dir, 'Figures')):
        os.mkdir(os.join(save_dir, 'Figures'))
    ...
    plt.savefig(os.path.join(save_dir, 'Figures', 'Fig11.png'))
    plt.show()
