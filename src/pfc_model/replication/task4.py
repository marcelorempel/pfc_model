...
def task4(simulation_dir, seed=None):
    ...
    _fig05(ISImean_list, ISICV_list, crossC_mean_list, C_lags,
    ...
    _fig06(autoC_mean_list, C_lags, simulation_dir)
    ...
    _fig07(fq_list, pwr_list, simulation_dir)
...
def _fig04(cortex_rk4, cortex_rk2, tlim, path):
...
def _fig05(ISImean_list, ISICV_list, crossC_mean_list, C_lags,
...
def _fig06(autoC_mean_list, C_lags, path):
    if not os.path.isdir(os.path.join(path, 'Figures')):
        os.mkdir(os.path.join(path, 'Figures'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(path, 'Figures', 'Fig06.png'))
    
    
def _fig07(frequency_list, power_list, path):
...
def _fig08(VstdALL, VstdPC, path):
    if not os.path.isdir(os.path.join(path, 'Figures')):
        os.mkdir(os.path.join(path, 'Figures'))
    ...
    ax.text(0, 1, '(A)', transform=ax.transAxes + trans,
        fontsize=24, verticalalignment='top',
        bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=3.0))   
    plt.xlabel('V standard deviation (mV) - PC', fontsize=26)
    ...
    plt.xlabel('V standard deviation (mV) - all cells', fontsize=26)
    plt.ylabel('density', fontsize=26)    
    sns.kdeplot(VstdALL, clip=[0, np.NaN])
    plt.tick_params(labelsize=26)
    plt.locator_params(axis='both', nbins=4)
    plt.xlim(0, 12)
